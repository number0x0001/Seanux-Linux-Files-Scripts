...
<td style='padding-left:10px;' >Web Scanner</td></tr><tr><td height='45' colspan='2'>
<input type="text" name="dork" value="<?php //if($_POST['dork']){echo filter($_POST['dork']);}else{echo "127.0.0.1";} ?>" size="70" />
<input type='submit' value='Scan Now' name='Scan'>
</td></tr></table></form>
<!-- End Of Bing Grabber -->
</center>
<?php
...
if($_POST['Scan'])
{
	if(!$_POST['dork']){echo "<tr><td width='500'><sy>Seems you forgot the target link ! .</sy></td></tr>";}
	else
	{
		$array = array();
		$link = trim($_POST['dork']);
		$link = str_replace(array("http://","www.","https://"),"",$link);
		$link = "http://".$link;
		$host = getHost($link);
		getAllLinks($link);
	}
...
{  
	...
	if($content)
	{
		...
		{
			if(strpos($Url,"?",0)){$Url = substr($Url,0,strpos($Url,"?",0));}
			if($Url[0] == "/" || $Url[0] == "\\"){$Url = deleteFirstChar($Url);}
			if(!in_array($Url,$array))
			{
				if(!strpos($Url,":",0))
				{
					array_push($array,$Url);
					echo  "<tr><td><sy>".$host."/<font color='white'>".$Url."</font><sy></td></tr>";
					flush();ob_flush();
					getAllLinks("http://www.".$host."/".$Url);
				}
			}
		}
...
function getHost($host)
{
	...
	$host = substr($host,0,strpos($host,"/",0));	
	return $host;
}
